...
import almacenarPaginaWeb
import almacenarPaginaWeb2
import almacenarPaginaWeb3
import ajustarValores

#------------------------------------------ FUNCIONES ------------------------------------------------------------
# Función para subir datos a Firestore
def subirDatosFirestore(collection_name, document_id, data, db):
    """
...
def obtenerNombreArchivo(marcaTiempo):
    return "/home/xibernetiq/Documents/Tesis/Interfaz/ArchivosTexto/PruebaFuncionamiento2/Dia_{}.txt".format(marcaTiempo.strftime("%Y-%m-%d"))
...
#----------------------------------------  LECTURA DE SENSORES ---------------------------------------- 
def lecturaSensores(varCompartidas, UVOUT, BATERYOUT, outRelay, dhtDeviceExterior, dhtDeviceBateria, ads, ser, i2c, interfaz, db):
    ...
    tiempoAlmacenarInicio = time.time() #Marca de tiempo para almacenar datos en archivos .TXT.
    tiempoAlmacenarFin = 0 #Marca de tiempo para almacenar datos en archivos .TXT.
        ...
        try:
            ...
            if horaInicio <= horaActual <= horaFin:
                if tiempoActual - tiempoImpresion >= 2:
                    ...
                    subirDatosFirestore("real_time", "datos_actuales", data_real_time, db)
                    subirDatosFirestore("historial", "null", data_historial, db)
                    tiempoActual = time.time()
                    tiempoImpresion = tiempoActual
            
            #------------------------------------------------------------------------------------------------------------
            tiempoAlmacenarFin = time.time()
            if (tiempoAlmacenarFin - tiempoAlmacenarInicio >= 60):
                now = datetime.datetime.now()
                marcaTiempo = now.strftime("%Y-%m-%d %H:%M")  # Formato de fecha y hora

                #Guardar datos del primer par de páginas web:
                almacenarPaginaWeb.main(now, marcaTiempo)

                # Guardar datos del segundo par de páginas web:
                almacenarPaginaWeb2.main(now, marcaTiempo)

                #Guardar datos de la última página web:
                almacenarPaginaWeb3.main(now, marcaTiempo)
                
                temperaturaWP = almacenarPaginaWeb.temperatura
                humedadWP = almacenarPaginaWeb.humedad

                data= "{} TemperaturaBateria(°C)= {:0.3f} TemperaturaExteriorTotemAjustado(°C)= {:0.3f} HumedadTotemAjustado(%)= {:.1f} TemperaturaExteriorTotemSinAjustar(°C)= {:0.3f} HumedadTotemSinAjustar(%)= {:.1f} PM10Totem(µg/m³)= {:.2f} PM2,5Totem(µg/m³)= {:.2f} IUVTotem= {} TemperaturaWP(°C)= {} HumedadWP= {} VoltajeBatería(V): {:0.2f}\n".format(marcaTiempo, varCompartidas["temperaturaCAjustada"], varCompartidas["humedadC"], temperatura2, temperaturaC, humedad, varCompartidas ["pm10"], varCompartidas["pm25"], round(varCompartidas["uvIntensity"]), temperaturaWP, humedadWP, voltajeBateria)
                
                #Guardar datos
                fileName = obtenerNombreArchivo(now)
                with open(fileName, "a") as fileDatos:  # Abre el archivo en modo "append"
                    fileDatos.write(data)

                tiempoAlmacenarFin = time.time()
                tiempoAlmacenarInicio = time.time()
