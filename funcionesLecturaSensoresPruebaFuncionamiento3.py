...
#------------------------------------------ FUNCIONES ------------------------------------------------------------
#Función para almacenar datos de voltaje y temperatura de batería
def almacenarVoltajeTXT(voltajeDivisor, voltajeBateriaCalculado, marcaTiempo, nowVoltaje, temperaturaBateria): 
    data = "{} VoltajeDivisorVoltaje(V): {:0.4f}  VoltajeBatería(V): {:0.4f} TemperaturaBatería(V): {:0.3f}\n".format(marcaTiempo, voltajeDivisor, voltajeBateriaCalculado, temperaturaBateria)
    fileNameVoltage = "/home/xibernetiq/Documents/Tesis/Interfaz/ArchivosTexto/VoltajeBateria/Dia_{}.txt".format(nowVoltaje.strftime("%Y-%m-%d"))
    with open(fileNameVoltage, "a") as file0:  # Abre el archivo en modo "append"
        file0.write(data)
...
def lecturaSensores(varCompartidas, UVOUT, BATERYOUT, outRelay, dhtDeviceExterior, dhtDeviceBateria, ads, ser, i2c, interfaz, db):
    ...
    while varCompartidas["indicadorParada"] == False:
        try:
            ...
            tiempoActual = time.time()
            if (tiempoActual - tiempoBateria >= 60) or (temperatura2 >= 47): #Medición de voltaje cada minuto.
                ...
                outRelay.value = True
                nowVoltaje = datetime.datetime.now()
                marcaTiempoVoltaje = nowVoltaje.strftime("%Y-%m-%d %H:%M")  # Formato de fecha y hora
                almacenarVoltajeTXT(voltajeChan0, voltajeBateria, marcaTiempoVoltaje, nowVoltaje, temperatura2)
                tiempoBateria = time.time()
                tiempoActual = time.time()
                if (voltajeBateria<=7.2) or (temperatura2 >= 47): #Cerrar aplicación y apagar placa cuando el voltje de batería sea menor o igual a 10,55 V o cuando la temperatura de la batería sea mayor o igual a 56 C.
